...
public class ManageStudent {
    ...
    try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName))) {
        for (Student student : students) {
            writer.write(student.getId() + "," + student.getName() + "," + student.getYearOfBirth());
            writer.newLine();
        }
    ...
    try (BufferedReader reader = new BufferedReader(new FileReader(fileName))) {
        ...
        while ((line = reader.readLine()) != null) {
            ...
            if (data.length == 3) {
                ...
                int yearOfBirth = Integer.parseInt(data[2].trim());
                students.add(new Student(id, name, yearOfBirth));
            }
        }
        System.out.println("Danh sách sinh viên đã được đọc từ file " + fileName + ".");

        // In danh sách sinh viên sau khi đọc xong
        printStudents();
